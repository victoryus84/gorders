...
func SetupRoutes(router *gin.Engine, hdl *handler.Handlers) {
	...
	api.Use(middleware.AuthJWT())
	{
		clients := api.Group("/clients")
		{
			...
			clients.POST("/addresses", hdl.Client.CreateClientAddress)
		...
		{
		products.GET("", hdl.Product.GetProducts)
		products.POST("", hdl.Product.CreateProduct)
		products.POST("/groups", hdl.Product.CreateProductGroup)
		}
	}
}
